...
if (preg_match('/set_(.*)/',$action,$reg))
{
	...
	if (dolibarr_set_const($db, $code, GETPOST($code, 'none'), 'chaine', 0, '', $conf->entity) > 0)
	{
		...
		if($code == 'NOMENCLATURE_DETAILS_TAB_REWRITE' && !getDolGlobalString('PRODUCT_USE_UNITS')){
...
if(getDolGlobalString('NOMENCLATURE_DETAILS_TAB_REWRITE') && !getDolGlobalString('PRODUCT_USE_UNITS')){
	// Lorsque la configuration "Séparer les produits des services dans l'onglet de détail des ouvrages" est activée dans nomenclature,
	// activer également la conf cachée "PRODUCT_USE_UNITS" si elle ne l'est pas déjà.
...
if(getDolGlobalString('NOMENCLATURE_PERSO_PRICE_HAS_TO_BE_CHARGED')) {
...
if (getDolGlobalString('NOMENCLATURE_COST_TYPE') && $conf->global->NOMENCLATURE_COST_TYPE === '1') print 'checked ';
print '/> ';
...
if (getDolGlobalString('NOMENCLATURE_COST_TYPE') && $conf->global->NOMENCLATURE_COST_TYPE === 'pmp') print 'checked ';
print '/> ';
...
if (getDolGlobalString('NOMENCLATURE_COST_TYPE') && $conf->global->NOMENCLATURE_COST_TYPE === 'costprice') print 'checked ';
print '/> ';
