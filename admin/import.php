...
function _card(&$PDOdb) {
	...
	if(!getDolGlobalString('NOMENCLATURE_IMPORT_SEPARATOR')) $conf->global->NOMENCLATURE_IMPORT_SEPARATOR = ',';
...
function _import_to_session() {
	...
	if(GETPOST('bt_view', 'none') && !empty($_FILES['file1']['name'])) {
		...
		while(!feof($f1)) {
            ...
            $row = fgetcsv($f1, 4096, getDolGlobalString('NOMENCLATURE_IMPORT_SEPARATOR') ? $conf->global->NOMENCLATURE_IMPORT_SEPARATOR : ',', '"');
