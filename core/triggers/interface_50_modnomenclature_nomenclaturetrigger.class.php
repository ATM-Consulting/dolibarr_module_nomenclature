...
//
		...
		if($action == 'PRODUCT_CREATE' && in_array('createfromclone', $object->context) && getDolGlobalString('NOMENCLATURE_CLONE_ON_PRODUCT_CLONE')) {
	...
	private function _setPrice(&$PDOdb, &$object,$fk_parent,$object_type) {
		...
		if ($object->product_type > 1 || (!getDolGlobalString('NOMENCLATURE_USE_SELL_PRICE_INSTEADOF_CALC') && $object->subprice>0)) return 0; //si on ne prends systématique le PV mais que ce dernier est défini, alors il prend le pas. Pour que le prix calculé soit utilisé, il faut un PV = 0
		...
		if (getDolGlobalString('NOMENCLATURE_USE_SELL_PRICE_INSTEADOF_CALC')) {
			$sell_price_to_use=$object->subprice;
		}
		else if (getDolGlobalString('NOMENCLATURE_DONT_USE_NOMENCLATURE_SELL_PRICE')){
		    $sell_price_to_use = 0;
		}
		else {
