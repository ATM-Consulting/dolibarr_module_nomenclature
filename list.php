...
function linkToNomenclature($label = '',$nomenclature_id = '', $object_type = '', $object_id = '' ){
    ...
    if($object_type == 'product'){
        $url = dol_buildpath('nomenclature/nomenclature.php',1).'?fk_nomenclature='.intval($nomenclature_id ).'&amp;token='.$newToken.'&amp;fk_product='.intval($object_id).'#nomenclature'.intval($nomenclature_id );
    }
    else {
        $url = dol_buildpath('nomenclature/nomenclature-detail.php',1).'?id='.intval($object_id).'&amp;token='.$newToken.'&amp;object='.$object_type;
    }
