...
if (empty($reshook))
{
	...
	//var_dump( $fk_nomenclature,$k,$n->TNomenclatureWorkstation);
		...
		else
		{
			...
			if(getDolGlobalString('NOMENCLATURE_UNIQUE_TITLE') && TNomenclature::getRightToSaveTitle(GETPOST('title', 'alphanohtml')) <= 0 && !empty(GETPOST('title', 'alphanohtml'))) {
	...
	else if ($action == 'confirm_create_stock' && getDolGlobalString('NOMENCLATURE_ALLOW_MVT_STOCK_FROM_NOMEN'))
	{
	...
	<div class="tabsAction">
		...
		<?php
	       ...
	       //print $form->select_produits('', 'fk_clone_from_product', '', 0);
			...
			print ajax_autocompleter('', $htmlname, dol_buildpath('/nomenclature/ajax/products.php', 1), $urloption, (getDolGlobalString('PRODUIT_USE_SEARCH_TO_SELECT')?$conf->global->PRODUIT_USE_SEARCH_TO_SELECT:''), 0, array());
			print $langs->trans("RefOrLabel").' : ';
			print '<input type="text" size="20" name="search_'.$htmlname.'" id="search_'.$htmlname.'" value="" '.(!getDolGlobalString('NOMENCLATURE_DISABLE_AUTOFOCUS') ? 'autofocus' : '').' />';
	    ?>
	    <div class="inline-block divButAction">
...
//console.log(name);
		...
		<?php if(getDolGlobalString('NOMENCLATURE_ALLOW_USE_MANUAL_COEF')) { ?>
		...
		<?php if(getDolGlobalString('NOMENCLATURE_USE_LOSS_PERCENT')) { ?>
			...
			$("input[name*=qty_base], input[name*=loss_percent]").change(function() {
				...
				if(typeof $('input[name*=qty_base]').attr('name') !== 'undefined') {
					...
					<?php if(getDolGlobalString('MAIN_MAX_DECIMALS_UNIT')) { ?>
               ...
               if(count($TNomenclatureDet)>0) {
                   ...
                   <table width="100%" class="liste det-table">
                       ...
                       <tr class="liste_titre">
                           ...
                           <?php if(getDolGlobalString('PRODUCT_USE_UNITS')) { ?> <th class="liste_titre col_fk_unit" width="5%"><?php echo $langs->trans('Unit'); ?></th> <?php } ?>
                           <?php
								...
								if(!empty($conf->stock->enabled) && !getDolGlobalString('NOMENCLATURE_HIDE_STOCK_COLUMNS')) {
								   ?>
                           ...
                           <?php if(getDolGlobalString('NOMENCLATURE_USE_CUSTOM_BUYPRICE')) { ?> <th class="liste_titre col_buy_price" width="5%"><?php echo $langs->trans('BuyingPriceCustom'); ?></th> <?php } ?>
                           <?php if($user->hasRight('nomenclature', 'showPrice')) { ?>
                               ...
                               <?php if(getDolGlobalString('NOMENCLATURE_USE_COEF_ON_COUT_REVIENT')) { ?>
                           ...
                           <tr class="<?php echo $class ?>" rowid="<?php echo $det->getId(); ?>" id="line_<?php echo $det->getId(); ?>">
                               <td><?php
								   ...
								   if($det->fk_product > 0) print '<input type="hidden" name="TNomenclature['.$k.'][fk_product]" value="'.$det->fk_product.'"/>';
									...
									if(getDolGlobalString('NOMENCLATURE_ALLOW_TO_LINK_PRODUCT_TO_WORKSTATION')) {
	                                ...
	                                <?php if(getDolGlobalString('PRODUCT_USE_UNITS')) { ?>
		                               <td class="ligne_col_fk_unit nowrap" ><?php
										   ...
										   $det->warningUnitNotTheSameAsProduct = ($det->fk_unit != $p_nomdet->fk_unit);
		                               		...
		                               		if(getDolGlobalString('NOMENCLATURE_ALLOW_SELECT_FOR_PRODUCT_UNIT') || !empty($det->warningUnitNotTheSameAsProduct)){
								...
								if(!empty($conf->stock->enabled) && !getDolGlobalString('NOMENCLATURE_HIDE_STOCK_COLUMNS')) {
	                             ...
	                             <?php }
	                             	...
	                             	if(getDolGlobalString('NOMENCLATURE_USE_LOSS_PERCENT')) {
							   ...
							   if(getDolGlobalString('NOMENCLATURE_USE_CUSTOM_BUYPRICE')) {
	                            ...
	                            if($user->hasRight('nomenclature', 'showPrice')) {
                                    ...
                                    if(!empty($det->qty)){
                                        if (
                                            getDolGlobalString('NOMENCLATURE_APPLY_FULL_COST_NON_SECABLE')
                                            && is_object($sub_n)
                                            && $sub_n->non_secable
                                        )
                                        {
									...
									if(getDolGlobalString('NOMENCLATURE_ACTIVATE_DETAILS_COSTS')) {
                               ...
                               ?>
                                   ...
                                   print '</td>';
	                            	...
	                            	if(getDolGlobalString('NOMENCLATURE_USE_COEF_ON_COUT_REVIENT')) { ?>
					...
					?></tbody><tfoot><?php
				       ...
				       if($user->hasRight('nomenclature', 'showPrice')) {
				       		$colspan = 3;
							if(getDolGlobalString('FOURN_PRODUCT_AVAILABILITY') && getDolGlobalInt('FOURN_PRODUCT_AVAILABILITY') > 0) $colspan ++;
							if(empty($conf->stock->enabled)) $colspan -= 2;
							if(getDolGlobalString('PRODUCT_USE_UNITS')) $colspan ++;
							if(getDolGlobalString('NOMENCLATURE_USE_LOSS_PERCENT')) $colspan += 2;
							if(getDolGlobalString('NOMENCLATURE_USE_CUSTOM_BUYPRICE')) $colspan ++;
			?>
        ...
        <tr>
           <td colspan="6"><?php
               ...
               <tr class="liste_titre">
                   ...
                   <?php if (getDolGlobalString('NOMENCLATURE_USE_TIME_BEFORE_LAUNCH')) {?>
                   ...
                   <?php }?>
                   <?php if (getDolGlobalString('NOMENCLATURE_USE_TIME_PREPARE')) {?>
                   <th class="liste_titre" width="5%"><?php echo $langs->trans('QtyPrepare').img_info($langs->trans('QtyPrepareHelp')); ?></th>
                   <?php }?>
                   <?php if (getDolGlobalString('NOMENCLATURE_USE_TIME_DOING')) {?>
...
//					   var_dump($ws->getId(), $ws);
                       ...
                       <tr class="<?php echo $class ?>"  id="nomenclature-ws-item-k-<?php echo $k; ?>" rowid="<?php echo $ws->getId(); ?>">
                           ...
                           if($user->hasRight('nomenclature', 'showPrice')) {
							   ...
							   if(getDolGlobalString('NOMENCLATURE_ACTIVATE_DETAILS_COSTS') && !empty($conf->of->enabled)) {
...
<?php
				...
				?></tbody><tfoot><?php
					...
					$colspan = 5;
					if (!getDolGlobalString('NOMENCLATURE_USE_TIME_BEFORE_LAUNCH')) $colspan--;
					if (!getDolGlobalString('NOMENCLATURE_USE_TIME_PREPARE')) $colspan--;
					if (!getDolGlobalString('NOMENCLATURE_USE_TIME_DOING')) $colspan--;
                        ...
                        <div class="tabsAction">
                            <div>
                                ...
                                if ( $conf->workstationatm->enabled && ! $readonly ) {
                                    echo $formCore->combo( '', 'fk_new_workstation', TWorkstation::getWorstations( $PDOdb, false, getDolGlobalString('NOMENCLATURE_PRESELECT_FIRST_WS') ? false : true ), - 1 );
                                    ?>
                                    <div class="inline-block divButAction">
		...
		if($user->hasRight('nomenclature', 'showPrice'))
		{
            ...
            <tr  data-row="TotalRow" >
		       ...
		       if(getDolGlobalString('NOMENCLATURE_ACTIVATE_DETAILS_COSTS')) { ?>
		        ...
		        if(getDolGlobalString('NOMENCLATURE_HIDE_ADVISED_PRICE'))
				{
			...
			<td colspan="5">
				<div class="tabsAction">
                    ...
                    if (!$readonly) {
                        ...
                        if (!$json && !empty($conf->stock->enabled) && getDolGlobalString('NOMENCLATURE_ALLOW_MVT_STOCK_FROM_NOMEN')) {
...
function _draw_child_arbo(&$PDOdb, $id_product, $qty = 1, $level = 1) {
	...
	$max_level = !getDolGlobalString('NOMENCLATURE_MAX_NESTED_LEVEL') ? 50 : $conf->global->NOMENCLATURE_MAX_NESTED_LEVEL;
	if($level > $max_level) {
