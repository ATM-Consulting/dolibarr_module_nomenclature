...
if (! empty($action) && $action == 'fetch' && ! empty($id))
{
	...
	if ($ret > 0)
	{
		...
		// Price by qty
		if (! empty($price_by_qty_rowid) && $price_by_qty_rowid >= 1 && (getDolGlobalString('PRODUIT_CUSTOMER_PRICES_BY_QTY'))) 		// If we need a particular price related to qty
		{
		...
		// Multiprice
		if (! $found && isset($price_level) && $price_level >= 1 && (getDolGlobalString('PRODUIT_MULTIPRICES'))) 		// If we need a particular price
		                                                                                                           // level (from 1 to 6)
		{
